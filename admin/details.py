...
def details():
    ...
    def single_proj(emp):
        ...
        cursor = db.cursor()
        cursor.execute("SELECT * FROM project WHERE assign=? AND status=?", (emp, 'Not Done',))
    ...
    def single_proj_id(proj_id):
        db = sqlite3.connect(curr_d() + '/admin/oms.db')
        cursor = db.cursor()
        cursor.execute("SELECT * FROM project WHERE proj_id=? AND status=?", (proj_id, 'Not Done',))
        records = cursor.fetchall()
        for row in records:
            proj_Name_fd.insert(END, row[1])
    ...
    def single_proj_name(name):
        db = sqlite3.connect(curr_d() + '/admin/oms.db')
        cursor = db.cursor()
        cursor.execute("SELECT * FROM project WHERE proj_name=? AND status=?", (name, 'Not Done',))
        records = cursor.fetchall()
        for row in records:
            proj_id_fd.insert(END, row[0])
            emp_id_fd.insert(END, row[2])
    ...
    def fill(emp_id_a, emp_name_a, proj_id, proj_name):
        ...
        if emp_id_a != "":
            if instance.search_by_emp_id(int(emp_id_fd.get())):
